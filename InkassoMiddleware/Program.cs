...
{
    // Use the MSNSC endpoint configuration (NoSc = No Security Context)
    var client = new IcelandicOnlineBankingClaimsSoapClient(
        IcelandicOnlineBankingClaimsSoapClient.EndpointConfiguration.MSNSC);
    
    // Set credentials from environment variables or defaults
    client.ClientCredentials.UserName.UserName = 
        Environment.GetEnvironmentVariable("INKASSO_USERNAME") ?? "";
    client.ClientCredentials.UserName.Password = 
        Environment.GetEnvironmentVariable("INKASSO_PASSWORD") ?? "";
...
// Create claims endpoint - uses raw SOAP client
app.MapPost("/CreateClaims", async (List<ClaimBatchItem> claims) =>
{
    try
    {
        if (claims == null || claims.Count == 0)
        {
            return Results.BadRequest(new { message = "At least one claim must be provided." });
        }

        string soapResponse = await InkassoRawSoapClient.CreateClaimsAsync(claims);
...
// Get operational result endpoint - uses raw SOAP client
app.MapPost("/GetOperationalResult", async (GetOperationalResultRequest req) =>
{
    try
    {
        if (string.IsNullOrWhiteSpace(req.OperationId))
        {
            return Results.BadRequest(new { message = "OperationId is required." });
        }

        string soapResponse = await InkassoRawSoapClient.GetClaimOperationResultAsync(req.OperationId);
...
// Query claims endpoint - Updated to use raw SOAP client
app.MapPost("/query-claims", async (ClaimQueryRequest req) =>
{
    try
    {
        // Use the raw SOAP client which is working
        string soapResponse = await InkassoRawSoapClient.QueryClaimsAsync(
            req.ClaimantId, 
            req.FromDate, 
            req.ToDate
        );
...
// Add new endpoint that uses the raw SOAP client with environment variables
app.MapPost("/query-claims-raw", async (ClaimQueryRequest req) =>
{
    try
    {
        // Create a StringWriter to capture console output
        ...
        // Query claims using the raw SOAP client
        string soapResponse = await InkassoRawSoapClient.QueryClaimsAsync(
            req.ClaimantId, 
            req.FromDate, 
            req.ToDate
        ...
        // Get the captured output
        string debugOutput = stringWriter.ToString();
    ...
    catch (Exception ex)
    {
        return Results.Problem(
            statusCode: 500,
            title: "SOAP Query Error",
            detail: $"Failed to query claims. Error: {ex.Message}"
        );
    }
});
